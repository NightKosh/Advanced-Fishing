...
public class SmeltingRecipesTests {
    ...
    protected static void defaultTest(GameTestHelper helper, String recipeName, Item input, ItemStack expected) {
        ...
        var res = new ResourceLocation(ModInfo.ID, "smelting/" + recipeName);
        ...
        if (!(recipeByKey.get().value() instanceof SmeltingRecipe)) {
            helper.fail("Recipe " + recipeName + " isn't an instance of SmeltingRecipe: " + recipeByKey.get().value());
            return;
        }
        ...
        var recipeOpt = level.getRecipeManager().getRecipeFor(RecipeType.SMELTING, container, level);
        if (recipeOpt.isEmpty()) {
            helper.fail("Can't find " + recipeName + " smelting recipe.");
            return;
        }
        ...
        if (recipe.getCookingTime() != 200) {
            helper.fail("Expected cookingTime = 200, but get " + recipe.getCookingTime());
            return;
        }

        LOGGER.info("Test for " + recipeName + " smelting recipe passed!");
