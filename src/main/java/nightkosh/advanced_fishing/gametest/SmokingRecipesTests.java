...
public class SmokingRecipesTests {
    ...
    protected static void defaultTest(GameTestHelper helper, String recipeName, Item input, ItemStack expected) {
        ...
        var recipeOpt = level.getRecipeManager().getRecipeFor(RecipeType.SMOKING, container, level);
        if (recipeOpt.isEmpty()) {
            helper.fail("Can't find " + recipeName + " smoking recipe.");
            return;
        }
        ...
        if (recipe.getCookingTime() != 100) {
            helper.fail("Expected cookingTime = 100, but get " + recipe.getCookingTime());
            return;
        }

        LOGGER.info("Test for " + recipeName + " smoking recipe passed!");
